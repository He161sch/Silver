...
class GameConfigSpec extends AnyWordSpec with Matchers {
  "A GameConfig" when {
    "new" should {
      ...
      "getLastPlayerName" in {
        val config = GameConfig(Vector(Player("SETest", Hand(Vector(Card(1), Card(1))), Card(0)), Player("test", Hand(Vector(Card(0))), Card(0))), deck: Deck, 1, Vector[Player]())
        config.getLastPlayerName.name should be ("SETest")
      }
    }
  }
}
