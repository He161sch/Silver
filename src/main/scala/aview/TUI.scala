    ...
    inputsplit match{
      case _ =>
        if (inputsplit.head.matches("d") && inputsplit(1).matches("1|2|3|4|0")){
          ...
          val newPlayer = controller.switchCard(player, inputsplit(1).toInt, newCard)
          return newPlayer
          ...
          val newPlayer = controller.combineCard(player, inputsplit(1).toInt, inputsplit(2).toInt, newCard)
          return newPlayer
        }
        player
    }
...
//  }
  ...
  override def update: Unit = println(controller.playerToString)
}